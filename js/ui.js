...
<th class="px-2.5 py-1.5 border dark:border-slate-600 bg-violet-100 dark:bg-violet-900 text-violet-700 font-bold border-l-2 border-slate-200 dark:border-slate-600 text-center sticky top-0 z-20">App Use %</th>
        ...
        `;
            ...
            filterAdminCustomers() {
                ...
                if (countEl) {
                    countEl.innerHTML = `Showing <span class="text-brand-600 dark:text-brand-400 font-extrabold">${Math.min(filtered.length, 300)}</span> of <span class="font-bold">${filtered.length}</span> matching customers (${customers.length} total)`;
                }
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                                ...
                                <td class="px-3 py-2 text-center sticky right-0 bg-white dark:bg-slate-900 z-10">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <button onclick="UI.openAddCustomerCollectionModal('${custKey}')" class="p-1.5 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 rounded transition" title="Log / Edit Collection">
                                            <i class="fa-solid fa-money-bill-wave text-xs"></i>
                                        </button>
                                    </div>
                                </td>
        ...
        `;
            ...
            openCollectionModal(id = null, prefillTId = null) {
                ...
                const html = `
                    <form onsubmit="UI.saveManualCollection(event)" class="space-y-3 text-xs">
                        ...
                        <!-- REGULAR & ADVANCE AMOUNTS WITH LIVE TOTAL DISPLAY -->
                        <div class="p-3 bg-emerald-500/5 dark:bg-emerald-950/20 rounded-2xl border border-emerald-500/20 space-y-2.5">
                            <div class="flex justify-between items-center pb-1.5 border-b border-emerald-500/20">
                                <span class="text-xs font-black text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                                    <i class="fa-solid fa-coins text-emerald-500"></i> Collection Amounts
                                </span>
                                <span class="text-xs font-bold text-slate-500">Total: <strong id="coll-modal-live-total" class="font-mono text-emerald-600 dark:text-emerald-400 font-black text-base">৳${((parseFloat(item.regularAmount) || 0) + (parseFloat(item.advanceAmount) || 0)).toLocaleString()}</strong></span>
                            </div>

                            <div class="grid grid-cols-2 gap-2.5">
                                <div>
                                    <label class="block text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase mb-1">Regular (৳)</label>
                                    <input type="number" step="any" name="regularAmount" id="coll-input-regular" oninput="UI.calcCollModalTotal()" value="${item.regularAmount || 0}" class="w-full h-10 px-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 text-right font-mono font-black text-slate-800 dark:text-slate-100 text-sm">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase mb-1">Advance (৳)</label>
                                    <input type="number" step="any" name="advanceAmount" id="coll-input-advance" oninput="UI.calcCollModalTotal()" value="${item.advanceAmount || 0}" class="w-full h-10 px-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 text-right font-mono font-black text-slate-800 dark:text-slate-100 text-sm">
                                </div>
                            </div>
                        </div>
            ...
            calcCollModalTotal() {
                const reg = parseFloat(document.getElementById('coll-input-regular')?.value) || 0;
                const adv = parseFloat(document.getElementById('coll-input-advance')?.value) || 0;
                const totalEl = document.getElementById('coll-modal-live-total');
                if (totalEl) {
                    totalEl.textContent = `৳${(reg + adv).toLocaleString()}`;
                }
            },
